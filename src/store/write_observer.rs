...
impl WriteObserver {
    #[inline]
    pub(super) fn group_started(&self, _exports: usize) {
        #[cfg(feature = "benchmark-support")]
        {
    ...
    #[inline]
    pub(super) fn ingest_only_transaction(&self) -> TransactionObservation<'_> {
        self.transaction(TransactionKind::IngestOnly)
    }
    ...
    #[inline]
    pub(super) fn retention_only_transaction(&self) -> TransactionObservation<'_> {
        self.transaction(TransactionKind::RetentionOnly)
    }
    ...
    fn transaction(&self, _kind: TransactionKind) -> TransactionObservation<'_> {
        #[cfg(feature = "benchmark-support")]
        {
            ...
            match _kind {
                TransactionKind::IngestOnly => {
                    saturating_increment(&self.counters.ingest_only_transactions_started);
                }
                TransactionKind::RetentionOnly => {
                    saturating_increment(&self.counters.retention_only_transactions_started);
                }
            }
        }
    ...
    pub(super) fn snapshot(&self) -> BenchmarkSnapshot {
        BenchmarkSnapshot {
            ...
            ingest_only_transactions_started: load(&self.counters.ingest_only_transactions_started),
            ingest_only_transactions_committed: load(
                &self.counters.ingest_only_transactions_committed,
            ),
            ingest_only_transactions_not_committed: load(
                &self.counters.ingest_only_transactions_not_committed,
            ),
            ...
            retention_only_transactions_started: load(
                &self.counters.retention_only_transactions_started,
            ),
            retention_only_transactions_committed: load(
                &self.counters.retention_only_transactions_committed,
            ),
            retention_only_transactions_not_committed: load(
                &self.counters.retention_only_transactions_not_committed,
            ),
...
impl TransactionObservation<'_> {
    ...
    pub(super) fn committed(mut self) {
        ...
        match self.kind {
            TransactionKind::IngestOnly => {
                saturating_increment(&self.observer.counters.ingest_only_transactions_committed);
            }
            TransactionKind::RetentionOnly => {
                saturating_increment(&self.observer.counters.retention_only_transactions_committed);
            }
        }
    }
}
            ...
            TransactionKind::IngestOnly => {
                saturating_increment(
                    &self
                        .observer
                        .counters
                        .ingest_only_transactions_not_committed,
                );
            }
            TransactionKind::RetentionOnly => {
                saturating_increment(
                    &self
                        .observer
                        .counters
                        .retention_only_transactions_not_committed,
                );
            }
        }
    }
...
struct BenchmarkCounters {
    ...
    ingest_only_transactions_started: AtomicU64,
    ingest_only_transactions_committed: AtomicU64,
    ingest_only_transactions_not_committed: AtomicU64,
    retention_invocations: AtomicU64,
    retention_failures: AtomicU64,
    retention_elapsed_ns: AtomicU64,
    retention_only_transactions_started: AtomicU64,
    retention_only_transactions_committed: AtomicU64,
    retention_only_transactions_not_committed: AtomicU64,
